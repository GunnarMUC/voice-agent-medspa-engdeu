...
### Console-Local Mode (empfohlen, vollständig offline)
...
│  + VAD          │     └──────────┘     └─────────┘     └──────────┘
└─────────────────┘                           │
                                              ├──▶ Booking API (Function Calling)
                                              └──▶ Konversationsverlauf
```

### LiveKit Mode (optional, cloud-fähig)

```
LiveKit Server → STT (konfigurierbar) → LLM (Ollama) → TTS (konfigurierbar)
                                              │
                                              └──▶ Booking Tools
```

Die Standard-STT/TTS im LiveKit-Modus nutzt AssemblyAI/Cartesia (Cloud). Für vollständig lokalen Betrieb eigene STT/TTS-Plugins implementieren oder den Console-Local Mode verwenden.
...
Das Setup-Skript erstellt eine virtuelle Umgebung, installiert alle Abhängigkeiten und lädt die TTS-Modelle herunter.
...
| `LIVEKIT_STT_MODEL` | STT für LiveKit-Mode | `assemblyai/universal-streaming-multilingual` |
| `LIVEKIT_TTS_MODEL` | TTS für LiveKit-Mode | Cartesia sonic-3 |
...
│   ├── llm.py             # LLM (Ollama, inkl. Tool-Calling)
│   ├── rag.py             # RAG-Engine (ChromaDB)
│   └── prompts.py         # System-Prompts + Booking-Tool-Definitionen
...
│   ├── run_agent_console.sh
│   ├── run_dashboard.sh
│   ├── smoke_test.py
│   └── test_pipeline_one_turn.py
...
Apache License 2.0 – siehe [LICENSE](LICENSE) im Projektroot.
